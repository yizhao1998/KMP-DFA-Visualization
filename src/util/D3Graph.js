...
export default () => {
    ...
    // Read dummy data
    d3.json(process.env.PUBLIC_URL + "/mock/data.json").then(function (
        data
    ) {
        // List of node names
        ...
        const nodeMouseOverHighlight = (d) => {
        ...
        const nodeMouseLeaveDeHighlight = (d) => {
        ...
        [nodes, labels].forEach((x) => {
            x.on("mouseover", function (event, d) {
                nodeMouseOverHighlight.call(that, d);
        ...
        const linkMouseOverHighlight = (d) => {
            // Highlight the nodes: every node is green except of him
        ...
        const linkMouseLeaveDeHighlight = (d) => {
            nodes.style("fill", "#69b3a2");
            links.style("stroke", "black").style("stroke-width", "1");
            selfLinks.style("stroke", "green").style("stroke-width", "1");
        };
